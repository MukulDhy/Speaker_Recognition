...
class SpeakerRecognitionSystem:
    ...
    def _process_audio_segment(self, audio_segment):
        """Process an audio segment for speaker identification"""
        try:
    ...
    def batch_process_recordings(self, directory):
        """Batch process multiple recordings for improved efficiency"""
        if not self.active_user:
            print("No active user selected.")
            return
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            for wav_file in wav_files:
                futures.append(executor.submit(self.identify_speaker, wav_file))
            ...
            for i, future in enumerate(concurrent.futures.as_completed(futures)):
                ...
                if result:
                    speaker, confidence = result
                    results[os.path.basename(wav_files[i])] = {"speaker": speaker, "confidence": confidence}
                else:
                    results[os.path.basename(wav_files[i])] = {"speaker": "Unknown", "confidence": 0.0}
        ...
        speaker_counts = Counter([r["speaker"] for r in results.values() if r["speaker"] != "Unknown"])
        
        for speaker, count in speaker_counts.most_common():
            print(f"{speaker}: {count} recordings")
        
        if "Unknown" in [r["speaker"] for r in results.values()]:
            unknown_count = sum(1 for r in results.values() if r["speaker"] == "Unknown")
            print(f"Unknown: {unknown_count} recordings")
...
def main():
    ...
    while True:
        ...
        print("8. Batch process recordings")
        print("9. Tune recognition parameters")
        print("10. Export model")
        print("11. Optimize for device")
        print("0. Exit")
        ...
        if choice == '1':
            ...
            directory = input("Enter directory path containing audio files: ")
            system.batch_process_recordings(directory)
